...
export default function AddProjectForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await action(fd); // server action call
      if (!result.success) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl mx-auto p-4 bg-neutral-600 rounded shadow">
      ...
      <label className="block mb-3">
        ...
        <input name="github_url" type="url" className="mt-1 block w-full border rounded px-3 py-2" required />
      </label>
      ...
      <div className="mt-4 flex items-center gap-3">
        <button disabled={loading} type="submit" className="px-4 py-2 bg-violet-700 text-white rounded">
